...
int main(int argc, char *argv[]) {
  ...
  switch (threads) {
    case 1024:
      volume_calculate_gpu<1024> <<<blocks,threads,sizeof(float)*threads>>>(gpu_mesh, gpu_result);
      break;
    case 512:
      volume_calculate_gpu<512> <<<blocks,threads,sizeof(float)*threads>>>(gpu_mesh, gpu_result);
      break;
    case 256:
      volume_calculate_gpu<256> <<<blocks,threads,sizeof(float)*threads>>>(gpu_mesh, gpu_result);
      break;
    case 128:
      volume_calculate_gpu<128> <<<blocks,threads,sizeof(float)*threads>>>(gpu_mesh, gpu_result);
      break;
    case 64:
      volume_calculate_gpu<64> <<<blocks,threads,sizeof(float)*threads>>>(gpu_mesh, gpu_result);
      break;
    case 32:
      volume_calculate_gpu<32> <<<blocks,threads,sizeof(float)*threads>>>(gpu_mesh, gpu_result);
      break;
  }
